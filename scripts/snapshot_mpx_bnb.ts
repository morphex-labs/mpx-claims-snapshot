...
async function snapshotHolders(snapshotBlock: number): Promise<MpxHolder[]> {
  ...
  let mpx = await ethers.getContractAt(mpxBnbAbi, MPX_BNB_ADDRESS, undefined);
...
async function changeLpToMpxAmounts(totalSupply: bigint) {
  let lpPair = holders.find(
    (h) => h.address.toLocaleLowerCase() == MPX_BNB_LP_PAIR_ADDRESS
  );
...
async function main() {
  ...
  try {
    console.log(
      `Getting MPX holders from data/mpx_bnb_raw_snapshot_${snapshotBlock}.json...`
    );
  ...
  holders = holders.filter(
    (holder) =>
      ...
      MPX_BNB_REWARD_TRACKER_ADDRESS.toLocaleLowerCase()
  );
  ...
  holders = holders.filter(
    (holder) =>
      holder.address.toLocaleLowerCase() !=
      MPX_BNB_LP_PAIR_ADDRESS.toLocaleLowerCase()
  );
  holders = holders.filter(
    (holder) =>
      holder.address.toLocaleLowerCase() !=
      MPX_BNB_LP_GAUGE_ADDRESS.toLocaleLowerCase()
  );
  ...
  for (var index in MPX_BNB_BLACKLIST) {
    holders = holders.filter(
      (holder) =>
        holder.address.toLowerCase() != MPX_BNB_BLACKLIST[index].toLowerCase()
    );
  }
